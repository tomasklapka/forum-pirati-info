        ...
        return new Promise((resolve, reject) => {
            const create_table_json_cache = `
                CREATE TABLE IF NOT EXISTS "json_cache" (
                    ...
                    CONSTRAINT "json_cache_pkey" PRIMARY KEY ("typeid", "id", "page")
                )
            `;
            const promises = [];
        ...
        return new Promise((resolve, reject) => {
            if (Scrapper.isCacheable(data.typeId)) {
                ...
                const store_query = `INSERT INTO "json_cache"
                    ("typeid", "id", "page", "url", "content", "version", "scrapped_at")
                    VALUES
                    ($1, $2, $3, $4, $5, $6, now()) ` +
                    `ON CONFLICT ON CONSTRAINT "json_cache_pkey" DO `+
                    `UPDATE SET "content" = $5, "version" = $6, "scrapped_at" = now(), "updated_at" = now()`;
                const values = [+data.typeId, id, page, data.url.href, JSON.stringify(data, 0).length, Scrapper.jsonVersion];
                debug('"%s"\nvalues: "%o"', store_query, values);
                values[4] = data;
                this.db.client.query(store_query, values).then(() => {
                    resolve();
                }).catch((err) => {
                    debug('JsonCache storing error / store (typeId="%d", id="%d", page="%d")', data.typeId, id, page);
                    reject(err);
                });
            } else {
                resolve();
    ...
    load(url) {
        return new Promise((resolve, reject) => {
            if (Scrapper.isCacheable(Scrapper.linkType(url))) {
                ...
                const select_query = `SELECT "content", "scrapped_at" FROM "json_cache" WHERE "typeid" = $1 AND "id" = $2 AND "page" = $3`;
                debug('"%s"\nvalues: "%o"\nurl: "%o"', select_query, [meta.typeId, meta.id || 0, meta.page], url);

                this.db.client.query(select_query, [meta.typeId, meta.id || 0, meta.page]).then((res) => {
                    ...
                    debug('JsonCache loading error (typeId="%d", id="%d", page="$d")', meta.typeid, meta.id, meta.page);
                    reject(err);
                });
            } else {
                resolve(null);
