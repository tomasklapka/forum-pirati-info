...
// named true - optional parameter for get_query_param_from_url(url, param, OnlyNonZero = false);
const ONLY_NON_ZERO = true;
...
class Scrapper {
    ...
    get(options) {
        ...
        options = options || { };
        const url = normalize_url(options.url);
    ...
    scrap(url) {
        ...
        return new Promise((resolve, reject) => {
            this.request({
                ...
                debug('saving and resolving scrapped data');
                this.saveToCache(sd).catch(debug);
                this.db.save(sd).catch(debug);
                resolve(sd);
            });
        });
    }
    ...
    scrapLinks(sd) {
        ...
        const $ = sd.$;
        $('a').each((i, a) => {
            let url = normalize_url($(a).attr('href'), this.baseOrigin);
            if (url) {
                // debug(url);
                url.hash = null;
                const meta = Scrapper.parseForumUrl(url);
                if (meta.typeId !== PageType.None) {
                    sd.links.push({
                        typeId: meta.typeId,
                        url: url,
    ...
    scrapForum(sd) {
        ...
        const $ = sd.$;
        this.scrapRules(sd);
        sd.id = Scrapper.forumIdFromUrl(sd.url);
        ...
        $('.topiclist').each((i, p) => {
            if ($(p).hasClass('topics')) {
                ...
                $(p).children('li').each((i, p) => {
                    ...
                    const topic = {
                        id: Scrapper.topicIdFromUrl(topicUrl),
                        urlBased: topicUrl,
                    ...
                    if ($(p).hasClass('global-announce')) {
                        sd.announcements[topic.id] = topic;
                        sd.sections[sectionIndex].announcements.push(topic.id);
                    } else {
                        sd.topics[topic.id] = topic;
                        sd.sections[sectionIndex].topics.push(topic.id);
                    }
                })
            } else {
                if ($(p).hasClass('forums')) {
                    ...
                    $(p).children('li').each((i, p) => {
                        ...
                        const forum = {
                            id: Scrapper.forumIdFromUrl(forumUrl),
                            urlBased: forumUrl,
                        ...
                        sd.forums[forum.id] = forum;
                        sd.sections[sectionIndex].forums.push(forum.id);
                    })
                } else {
    ...
    scrapTopic(sd) {
        ...
        $('div.post:has(div.postbody)').each((i, p) => {
            ...
            if (sd.id === null) {
                sd.id = Scrapper.topicIdFromUrl(postUrl);
                debug('scraping id: %o', sd.id);
            }
    ...
    scrapUser(sd) {
        ...
        const $ = sd.$;
        const user = sd.user;
        sd.id = Scrapper.userIdFromUrl(sd.url);
    ...
    scrapGroup(sd) {
        ...
        const $ = sd.$;
        sd.id = Scrapper.groupIdFromUrl(sd.url);
        const title = $('h2');
        sd.title = title.text();
        ...
        $('div.inner').children('table.table1').each((i, p) => {
            ...
            table.children('tbody').children('tr').each((i, e) => {
                ...
                user.id = Scrapper.userIdFromUrl(user.urlBased);
                td = td.next();
    ...
    static getPageFromUrl(url, typeId = null) {
        ...
        const start = get_query_param_from_url(url, 'start', ONLY_NON_ZERO);
    ...
    static parseForumUrl(url, typeId = null) {
        ...
        const meta = {
            typeId: typeId,
            id: Scrapper.getIdFromUrl(url, typeId),
    ...
    static forumIdFromUrl(url) {
        const f = get_query_param_from_url(url, 'f', ONLY_NON_ZERO);
        if (f) {
            return f;
        } else {
    ...
    static topicIdFromUrl(url) {
        return get_query_param_from_url(url, 't', ONLY_NON_ZERO);
    }

    static userIdFromUrl(url) {
        return get_query_param_from_url(url, 'u', ONLY_NON_ZERO);
    }

    static groupIdFromUrl(url) {
        return get_query_param_from_url(url, 'g', ONLY_NON_ZERO);
    }

    static postIdFromUrl(url) {
        return get_query_param_from_url(url, 'p', ONLY_NON_ZERO);
    }
    ...
    static getRedirectLocation(url) {
        return new Promise((resolve) => {
            Scrapper.request({
                url: url,
                method: 'HEAD',
