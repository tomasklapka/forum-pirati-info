...
const ForumScrapper = require('./scrapper');
...
class Db {
            ...
            this.client.connect().then(() => {
                const promises = [`
                    ...
                    CREATE TABLE IF NOT EXISTS "resources" (
                        ...
                        CONSTRAINT "resources_pkey" PRIMARY KEY ("phpbbid")
                    )
                    `
                ].map((query) => {
                    // debug(query);
    ...
    save(data) {
        ...
        switch (data.typeid) {
            ...
            case PageType.Topic:
                ...
                promises.push(this.save_posts(data.phpbbid, data.navi.page, data.posts));
                break;
            case PageType.Group:
                table = 'groups';
    ...
    save_posts(topic_phpbbid, page, posts) {
        debug('save_posts %d %d %d', topic_phpbbid, page, posts.length);
        return new Promise((resolve, reject) => {
            const promises = [];
    ...
    save_users(group_phpbbid, users) {
        ...
        return new Promise((resolve, reject) => {
            ...
            for(let i = 0; i < ids.length; i++) {
                if (ids[i] !== 'null') {
                    const values = [ group_phpbbid, ids[i] ];
