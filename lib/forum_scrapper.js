...
class ForumScrapper {
    ...
    constructor(url, base, originBase) {
    ...
    static scrap(url, base, originBase, requestCache) {
        return new Promise((resolve, reject) => {
            ...
            scrapper.scrap().then((data) => {
                ...
                debug("scrapper.scrap() error %o", err);
                reject(err);
            });
        });
    }
    ...
    static scrapPost(url) {
        ...
        return new Promise((resolve, reject) => {
            ForumScrapper.request({
                ...
                const content = $('textarea#message').text().replace(regexp_quote_open, '').replace(regexp_quote_close, '');
    ...
    scrapLastPost(p) {
        ...
        p.children('span').children('a').each((i, p) => {
            ...
            if (urlType === PageType.User) {
                ...
                lastPost.userColor = ForumScrapper.colorFromStyle(this.$(p));
            } else {
        ...
        this.$('.topiclist').each((i, p) => {
            if (this.$(p).hasClass('topics')) {
                ...
                this.$(p).children('li').each((i, p) => {
                    ...
                    const topic = {
                        phpbbid: ForumScrapper.topicIdFromUrl(topicUrl),
                        url: topicUrl,
                        title: topicTitle,
                    ...
                    if (this.$(p).hasClass('global-announce')) {
                        ...
                        this.topics[topic.phpbbid] = topic;
                        this.sections[sectionIndex].topics.push(topic.phpbbid);
                    }
                })
            } else {
    ...
    scrapTopic() {
        ...
        this.$('.post').each((i, p) => {
            ...
            if (authorBody.length > 0) {
                ...
                likesBody.each((j, a) => {
                    likes.push({
                        ...
                        userColor: ForumScrapper.colorFromStyle(this.$(a)),
                    });
            ...
            if (this.phpbbid === null) {
                this.phpbbid = ForumScrapper.topicIdFromUrl(postUrl);
                debug('scraping phpbbid: %o', this.phpbbid);
            }
            ...
            currentDd = currentDd.next();
            post.userRegistered = ForumScrapper.parseDate(currentDd.text().replace('Registrován: ', ''));
            if (post.phpbbid) {
        ...
        if (this.phpbbid === null) {
            this.status = 'No topic found at "'+this.url+'"';
            this.statusCode = 404;
        }
    }
    ...
    normalizeLinks(text) {
        if (text && text.length > 0) {
            return text
                ...
                .replace(new RegExp('url\\(images/local', 'g'), 'url(/images/local');
    ...
    static parseForumUrl(url) {
        ...
        switch(meta.typeid) {
            case PageType.Forum:
                ...
                break;
            case PageType.Topic:
                meta.phpbbid = ForumScrapper.topicIdFromUrl(url);
    ...
    static topicIdFromUrl(url) {
        return +ForumScrapper.nthMatch(url, /(topic|-t)(\d+)(-\d+)?\.html/, 2);
    }
    ...
    static resource(req, res) {
        const proxy_request = ForumScrapper.request(req.app.get('originBase') + req.originalUrl);
        debug('resource proxy request: "%s"', req.app.get('originBase') + req.originalUrl);
        ...
        if (filePath) {
            proxy_request.pipe(fs.createWriteStream(config.dataDir + filePath))
        }
...
ForumScrapper.requestTimeout = requestTimeout;
ForumScrapper.jsonVersion = jsonVersion;
