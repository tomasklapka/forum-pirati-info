    ...
    scrapThread() {
        ...
        this.$('.post').each((i, p) => {
            ...
            const post = {
                ...
                avatarSrc: avatarSrc,
                user_rank: profileFields.first().text(),
                user_posts:  +profileFields.first().next().next().text().replace('Příspěvky: ', ''),
                user_registered: ForumScrapper.parseDate(profileFields.first().next().next().next().text().replace('Registrován: ', '')),
                created: created,
...
//                        const groupUrl = this.rebase(this.makeGroupUrl(id, option.text()));
    ...
    static threadIdFromUrl(url) {
        return ForumScrapper.firstMatch(url, /-t(\d+)(-\d+)?\.html$/);
    }
