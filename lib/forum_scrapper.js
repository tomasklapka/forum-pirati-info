...
const PageType = {
    None: '0',
    Root: '1', // forum id=null
    Forum: '2', // forum id, page
    Thread: '3', // topic id, page
    Group: '4', // group id, page
    User: '5', // user id
    Search: '6',
    Unanswered: '7',
    ActiveTopics: '8',
...
class ForumScrapper {
    ...
    scrapThread() {
        ...
        if (moderatorsBody.length > 0) {
            moderatorsBody.children('a').each((i, p) => {
                const moderator = {
                    url: this.rebase(this.$(p).attr('href')),
                    title: this.$(p).text()
                };
    ...
    static parseForumUrl(url) {
        ...
        switch(meta.typeId) {
            case PageType.Forum:
                ...
                page = ForumScrapper.firstMatch(url, /-f\d+\/page(\d+)\.html$/);
                if (page) {
                    meta.page = page/100;
                } else {
                    meta.page = 0;
            ...
            case PageType.Thread:
                ...
                page = ForumScrapper.nthMatch(url, /(topic|post|-t)\d+-(\d+)\.html$/, 2);
                if (page) {
                    meta.page = page/10;
                } else {
                    meta.page = 0;
            ...
            case PageType.Group:
                ...
                page = ForumScrapper.firstMatch(url, /-g\d+-(\d+)\.html/);
                if (page) {
                    meta.page = page/100;
                } else {
                    meta.page = 0;
    ...
    static colorFromStyle($) {
        ...
        if (style) {
            return style.replace(/color:\s*/, '').replace(';', '');
        }
        return null;
    }
