...
class JsonCache {
    ...
    save(data) {
        return new Promise((resolve, reject) => {
            ...
            if (ForumScrapper.cacheableType(data.typeId)) {
                ...
                debug('paging');
                //debug(data);
                debug(data.navi.page);
                debug(data.navi.pages);
                ...
                debug('info "%s", "%d", "%d", "%o"', table, phpbbid, page, is_last_page);
                ...
                this.client.query(store_query, [phpbbid, page, is_last_page, data]).then(() => {
                    if (is_last_page && page > 0) {
                        ...
                        debug('update "%s", "%o"', update_last_page_query, [phpbbid, page-1]);
    ...
    load(url) {
        return new Promise((resolve, reject) => {
            debug(ForumScrapper.linkType(url));
            debug(ForumScrapper.cacheableType(ForumScrapper.linkType(url)));
    ...
    setPostUrl(postUrl, url) {
        ...
        const phpbbid = ForumScrapper.postIdFromUrl(postUrl);
        const update_query = `INSERT INTO "post_urls" ("phpbbid", "url") VALUES ($1, $2) ON CONFLICT DO NOTHING`; //ON CONSTRAINT "post_urls_pkey" DO UPDATE SET "url" = $1`;
