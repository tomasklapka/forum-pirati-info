    ...
    save(data) {
        return new Promise((resolve, reject) => {
            if (ForumScrapper.cacheableType(data.typeid)) {
                ...
                const store_query = `INSERT INTO "json_cache" ("typeid", "phpbbid", "page", "url", "content", "version", "scrapped_at")
                    VALUES ($1, $2, $3, $4, $5, $6, now()) ` +
                    `ON CONFLICT ON CONSTRAINT "json_cache_pkey" DO `+
                    `UPDATE SET "content" = $5, "version" = $6, "scrapped_at" = now(), "updated_at" = now()`;

                debug('"%s"\nvalues: "%o"', store_query, [data.typeid, phpbbid, page, data.url, JSON.stringify(data, 0).length, ForumScrapper.jsonVersion]);

                this.client.query(store_query, [data.typeid, phpbbid, page, data.url, data, ForumScrapper.jsonVersion]).then(() => {
                    resolve();
                }).catch((err) => {
    ...
    load(url) {
        return new Promise((resolve, reject) => {
            if (ForumScrapper.cacheableType(ForumScrapper.linkType(url))) {
                ...
                debug('"%s"\nvalues: "%o"', select_query, [meta.typeid, meta.phpbbid || 0, meta.page]);

                this.client.query(select_query, [meta.typeid, meta.phpbbid || 0, meta.page]).then((res) => {
                    if (res.rows.length > 0) {
                        ...
                        if (validUntil.isBefore(now) || !data.version || data.version < ForumScrapper.jsonVersion) {
                            debug('CACHE: INVALID');
                            data['invalid'] = true;
                        } else {
                            debug('CACHE: VALID');
